...
Section numbers below (`§N`) refer to `docs/architecture-plan.md` unless stated otherwise.
...
| Process model | Single process through M13; GPU/network/renderer splits at M14–M16; site isolation explicitly deferred | Don't add process-splitting code before its milestone — see §0 and §12. |
...
1. **Understand:** restate the request as a concrete requirement. If it's vague ("add tab freezing"), figure out which milestone/subsystem it actually belongs to (§10 Task Decomposition) before touching code.
2. **Inspect:** follow §9 below — read this file, the relevant part of `docs/architecture-plan.md`, and the actual current code (not just what the plan says *should* exist — verify what's really there).
3. **Plan:** identify affected crates, the message/API shape needed at each boundary, what's genuinely new vs. what already exists to extend, and what the Definition of Done (§14) looks like for this specific task before writing code.
4. **Implement:** smallest coherent change that satisfies the plan (§12). Follow §5's implementation rules.
5. **Integrate:** wire it into every real caller/consumer — see §13. A function that compiles but isn't called from anywhere real is not done.
6. **Validate:** run the checks in §15. Don't claim a check passed without running it.
7. **Review:** self-review the diff per §19 before reporting completion.
8. **Finish:** report per §27 — what was inspected, what changed, what was validated, what's still open.
...
8. Explicitly decide and state: does the requested functionality already partially exist? If yes, extend it; don't create a parallel implementation (§11).
...
- **Constraints:** which of §2's non-negotiables and §5's implementation rules apply?
- **Affected subsystems:** which crate(s), per §3/§4?
- **Implementation steps:** in the order they need to happen (respecting the dependency direction in §3).
- **Integration points:** every caller, config path, UI exposure, persistence path, and error path this touches (§13).
- **Validation criteria:** what specifically will you run/check to confirm it works (§15)?
...
- [ ] Integration is complete per §13 above — not just the isolated module.
...
- [ ] Relevant tests exist and pass (§15/§25) — new logic without any test coverage is not done, absent a stated reason (e.g., genuinely requires manual GPU/hardware verification).
...
- [ ] Documentation updated where §20 requires it.
...
- [ ] The dependency-direction check (§3) still passes if any crate boundary was touched.
...
cargo metadata --format-version 1 | jq '...'                # mandatory if any crate's Cargo.toml changed (§3 check)
...
- Identify the actual failure boundary (which crate, which message boundary, which thread) before guessing at a fix — use `tracing` output and the crate boundaries in §3/§4 to narrow it down.
...
- Make one logical change at a time and re-validate (§15) before making the next — don't batch multiple speculative fixes together.
...
- Forget storage schema migration when changing a persisted shape (§20/§13).
- Forget shutdown/cleanup paths (§13).
- Assume a feature is complete because it compiles — compilation success is a precondition for done, not evidence of done.
- Add a dependency because it's easier (§7).
- Perform a broad refactor for a small, targeted task (§12).
- Create a new file/crate in the wrong location instead of extending the crate that already owns that responsibility (§3/§10).
...
**Existing project architecture (§3/§4, `docs/architecture-plan.md`) > explicit requirements of the current task > established conventions already visible in the codebase > the minimal/simplest correct implementation > personal/stylistic preference.**

When uncertain:
- Inspect more code before guessing (§9).
...
- Document any assumption you had to make, explicitly, in your final report (§27).
...
- Did the implementation bypass an existing abstraction or architectural boundary (§3/§16)?
...
- Does the final state of the repository actually satisfy the original request — not just "does it compile," but "does §13's checklist fully pass"?
...
- **This file (`AGENTS.md`)** — if the change alters a rule, boundary, or workflow described here (e.g., a new crate added to §3, a new mandatory validation step).
...
- Keep commits/changes focused on the stated task (§11).
...
- Treat all network responses, parsed HTML/CSS/JS content, and IPC messages as **untrusted input** (§21/§8 of the plan) — validate before use, don't `unwrap()` on it (§5).
...
- **Regression tests:** every bug fix gets a fixture/test that reproduces the original failure, added permanently at the appropriate tier (§27 of the plan has the full hierarchy) — don't fix a bug without adding the regression test in the same change.
...
Skipping Integration or Testing does not count as reaching "done" — see §13's Definition of Done. A feature that's implemented but not integrated, or implemented but not tested, is a work-in-progress, and should be reported as such rather than as complete.
...
- State what validation was actually performed (§15) — and just as importantly, what validation was **not** performed and why (e.g., no GPU available in this environment).
...
- [ ] I searched for existing implementations before writing new code (§10).
- [ ] My change is the smallest coherent solution to the stated problem (§11).
...
- [ ] I wired the change into every real caller/config/persistence/error/shutdown path it needs (§13).
...
- [ ] I reviewed my own diff for unrelated changes, dead code, and debug artifacts (§19).
- [ ] I updated documentation where warranted (§20), and did not update it reflexively where not.
- [ ] My final report is honest about what was and wasn't validated (§27/§27 above), and about any assumptions made.