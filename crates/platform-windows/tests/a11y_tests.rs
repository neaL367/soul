...
fn test_uia_bridge_tree_and_hit_testing() {
    let mut root = UiaElement::new(
        1,
        UiaControlType::Document,
        "Document Root",
        (0.0, 0.0, 800.0, 600.0),
    );
    let heading = UiaElement::new(
        2,
        UiaControlType::Heading,
        "Welcome Page",
        (20.0, 20.0, 760.0, 40.0),
    );
    let button = UiaElement::new(
        3,
        UiaControlType::Button,
        "Submit Form",
        (20.0, 100.0, 120.0, 36.0),
    );
