...
///
/// `origin` is the page origin the context belongs to; it scopes every
/// database and record so sites never observe each other's storage.
///
/// # Errors
...
pub fn register_indexeddb(
    ...
    let put_fn = NativeFunction::from_copy_closure_with_captures(
        |_this, args, captures, ctx| {
            ...
            let _ = captures
                .store
                .put(&captures.origin, &db, &store, &key, &val);
    ...
    let get_fn = NativeFunction::from_copy_closure_with_captures(
        |_this, args, captures, ctx| {
            ...
            match captures.store.get(&captures.origin, &db, &store, &key) {
    ...
    let delete_fn = NativeFunction::from_copy_closure_with_captures(
        |_this, args, captures, ctx| {
            ...
            let _ = captures.store.delete(&captures.origin, &db, &store, &key);
            Ok(JsValue::undefined())
        },
        holder.clone(),
    );
    ...
    let open_fn = NativeFunction::from_copy_closure_with_captures(
        |_this, args, captures, ctx| {
            ...
            let _ = captures
                .store
                .open_or_create_db(&captures.origin, &db_name, version);
