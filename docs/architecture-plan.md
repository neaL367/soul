...
| M2 Window + input | OS input events, DPI | **Partial** — raw GPUI mouse/keyboard/wheel events route through `InputRouter`; GPUI bounds and scale factor emit `SoulEvent::WindowResized`; omnibox editing and navigation actions work; automated key injection and page hit-testing remain |
