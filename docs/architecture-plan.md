...
| M2 Window + input | OS input events, DPI | **Partial** — raw GPUI mouse/keyboard/wheel events now route through `InputRouter` and emit `SoulEvent::InputRouted`; omnibox editing and navigation submit work; resize/DPI forwarding, automated key injection, and page hit-testing remain |
...
**Status: Partial.** Raw GPUI elements now provide Soul toolbar buttons and an from-scratch omnibox; mouse, keyboard, and wheel events route through `InputRouter` and emit `SoulEvent::InputRouted`; keyboard editing and Enter navigation work; live frame polling refreshes post-start navigation frames. Outstanding: resize/DPI forwarding, automated key injection, and page hit-testing.
