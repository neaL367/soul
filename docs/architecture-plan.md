...
| M18 Media | MF playback + Canvas 2D | **Partial** — `ImageDecoder` (PNG/JPEG/WebP/GIF/BMP/SVG with `decode_auto` format sniffing and multi-frame animation delay extraction), `Canvas2DContext` real; `MediaPipeline` = state machine + solid-color `generate_frame`, no Media Foundation decode |
