...
- **Partial** — the milestone's components exist but lack key functionality, integration, or robust testing.
...
**Correction (2026-08-18):** a prior documentation pass rewrote the detailed per-milestone `**Status:**` lines below (M14–M18, M21–M23) to "Complete" without updating this table and without the underlying code changing — most visibly, M18's prose claimed `MfPlayer` "frame decoding" as complete when `MfPlayer::decode_frame` is a solid-color placeholder with no Media Foundation API calls anywhere in `crates/media/src/`. The table above was never wrong; the prose below it was. Both have now been reconciled back to the table's status, and M14/M15 (previously present in the prose but missing from this table entirely) have been added above. Going forward, if the table and a milestone's own prose disagree, **the table is correct and the prose is stale** — fix the prose, don't split the difference.
...
**Status: Partial.** `ipc` crate with command/event enums, length-prefixed JSON framing, async stream transports (`AsyncStreamTransport`), and named pipe connection helpers (`generate_pipe_name`, `accept_named_pipe_server`, `connect_named_pipe_client`) real and covered by roundtrip tests. Nothing outside the test suite opens a pipe yet — still single process, no GPU process exists.
...
**Status: Partial.** `NetworkService` supports both in-process async loops and out-of-process named pipe listeners (`run_named_pipe`), and streams chunked response messages over IPC in tests. `soul-shell`'s live navigation path still calls `networking` in-process, not over a pipe — no network process actually runs separately yet.
...
**Status: Partial.** Win32 Job Object (memory limits, UI lockdown, kill-on-close, `JobAccounting` queries via `JOBOBJECT_BASIC_ACCOUNTING_INFORMATION`), `RestrictedToken`, and `ProcessLauncher` (`SandboxedChild` keeps jobs alive) are real and tested in isolation. The restricted token is never actually applied to a spawned process, and nothing is launched into a job outside the test suite — no renderer runs sandboxed yet.
...
**M21 — Compositor & Performance Benchmarks *(renumbered)* — Status: Partial**
CPU `DamageTracker`/layer blits, GPU partial damage texture uploads (`upload_rgba_rect`), `GpuCompositor::composite_layers_with_damage`, dynamic viewport resizing, and the multi-iteration statistical benchmark harness (`benchmark_pipeline_stats`) are real and tested. Not yet done: the harness's output hasn't been checked against the actual targets in §28 — "the benchmark runs" and "the numbers meet target" are different claims, and only the first is verified.
Objective: compositor layer/damage infrastructure plus a repeatable multi-iteration pipeline benchmark harness. Components: `compositor`, `raster`, `benchmarks`. Depends on: M20. Tasks: damage tracking (dirty-rect accumulation), layer composition with transforms/opacity, GPU partial damage texture uploads (`upload_rgba_rect`), `GpuCompositor::composite_layers_with_damage`, dynamic viewport resizing, and multi-iteration statistical benchmark harness with FPS throughput metrics (`benchmark_pipeline_stats`). Tests: compositor unit tests, multi-iteration benchmark tests. DoD: layers composite with correct damage; the benchmark harness produces per-stage statistical metrics and FPS throughput **that meet or explain deviation from §28's targets**.

**M22 — Security Hardening**
**Status: Partial.** CSP Level 3 policy parser with nonce/hash sources and violation reporting (`CspViolationReport`), DPAPI-encrypted credential vault (`DpapiVault`), Windows 11 `AppsUseLightTheme` registry theme query in `platform-windows`, and `PrivateBrowsing` ephemeral profile are real. CORS + mixed-content are enforced on live subresources via `fetch_with_security_context`. Not yet verified: whether CSP actually *blocks* a resource load that violates a directive, versus only parsing the policy and reporting violations after the fact — reporting and enforcement are different guarantees, and only the former is confirmed.
