...
| M4 Networking | HTTP(S), DNS, redirects, cookies | **Partial** — real HTTP/1.1 + TLS 1.2/1.3 (hyper 1, rustls, webpki-roots) wired into shell live navigation; no hickory DNS, no redirects, no cookie jar, no HTTP/2 |
...
| M10a Software raster | CPU pixels on screen | **Partial** — `tiny-skia` rasterizer real; frames now reach disk as PNG via `browser-shell --output` (verified against a live site); still no window/screen presentation |
...
| M13 Storage | SQLite persistence | **Partial** — SQLite WAL (cookies/history/bookmarks/LocalStorage) + `register_storage` localStorage and `register_session_storage` sessionStorage JS bindings real + tested; not wired into the shell |
...
| M22 Security | CSP, DPAPI, private browsing | **Partial** — `CspPolicy` directive parse, `Dpapi`, `PrivateBrowsing` real; CORS + mixed-content now **enforced** in `HttpClient::fetch_with_security_context` (both checks + error types) but opt-in — zero non-test callers pass `document_origin`; no preflight/credentials support; no mixed-content blocking in the actual navigation path |
| M23 Production | signed installer, updates, crash reporting | **Partial** — shell wires the engine crates and now navigates live URLs end-to-end (fetch → parse → style → layout → paint → raster → PNG + a11y tree); `--type=` CLI flags are log-and-exit stubs, no child process spawned, no signing/updater/crash pipeline; workspace test suite green (109 passed) |

**Note (uncommitted wiring work):** `browser-shell` now has a real connected path — `NavigationController` drives live HTTP(S) fetches through the full rendering pipeline, with per-stage timings, PNG screenshot output, and accessibility-tree extraction (verified against `https://example.com`). New e2e integration tests live in `crates/browser-shell/tests/e2e_tests.rs` (local HTTP server → full pipeline → PNG + a11y assertions). Still unwired: named pipes, `ProcessLauncher`, `register_*` bindings, `fetch_with_security_context` (security checks exist on `HttpClient` but no subresource fetcher calls them yet).
...
**Status note (2026-08-14):** the M13-era milestone code exists and most items in this list are implemented and unit/integration-tested. The MVP is not yet *usable* as a browser: no real window opens (M1 simulated), and no pixels reach a screen (M10a renders to PNG on disk, not to a window). `browser-shell` now navigates live URLs end-to-end (fetch → parse → style → layout → paint → raster) — verified against a real HTTPS site — but M4 networking is not yet wired into subresource fetching, and the chrome remains headless. Treat this list as the capability contract; treat §31's status table as the current state.
