...
| M10b GPU compositor | wgpu compositor | **Partial** — wgpu 24 `GpuContext`/`GpuTexture` real with `GpuRect` damage region texture uploads (`upload_rgba_rect`) and `GpuCompositor::composite_layers_with_damage`; surface presentation via GPUI |
...
| M14 GPU split + IPC | GPU process, real IPC | **Partial** — ipc enums/framing/dispatcher real; tokio Windows named-pipe transports real + tested roundtrip with atomic `generate_pipe_name`; still single process — nothing uses pipes outside tests, no GPU process |
...
**M4 — Networking**
**Status: Partial.** Real HTTP/1.1 + TLS 1.2/1.3 client (hyper 1, rustls, webpki-roots). RFC 9110 HTTP payload decompression (`gzip`, `deflate` via `flate2`) with `Accept-Encoding` negotiation; bounded redirect following (5 hops, POST→GET on 301/302/303); `Set-Cookie` multi-header extraction and RFC 6265bis parsing into `storage::Cookie`; `fetch_with_security_context` enforcing CORS, CSP, and mixed-content blocking. Missing vs. plan: hickory-resolver DNS (OS resolver used), HTTP/2.
...
**Status: Partial.** `A11yNode` tree (roles, aria-label, bounds) real + tested; no UIA provider, not wired to `platform-windows`.
...
**M10b — GPU Compositor**
**Status: Partial.** `wgpu` 24 `GpuContext`/`GpuTexture` real with `GpuRect` damage region texture uploads (`upload_rgba_rect`) and `GpuCompositor::composite_layers_with_damage`; surface presentation via GPUI.
...
**Status: Partial.** `ipc` crate real: typed command/event enums, length-prefixed JSON framing, in-memory + generic stream transports, dispatcher, atomic-sequenced `generate_pipe_name` Windows named-pipe generation helper. Single process only — no OS named-pipe boundary, no GPU process.
...
**Status: Partial.** Win32 Job Object (memory limits, UI lockdown, kill-on-close, `JobAccounting` queries via `JOBOBJECT_BASIC_ACCOUNTING_INFORMATION`), `RestrictedToken`, and `ProcessLauncher` (`SandboxedChild` keeps jobs alive) real; no renderer process spawned into a Job/token yet outside tests.
...
**M18 — Media**
**Status: Partial.** `image-decode` (image crate + resvg/usvg: PNG, JPEG, WebP, GIF, ICO, BMP, multi-frame animation sequence decoding, `decode_auto` format sniffing, SVG rasterization) real; `Canvas2DContext` (tiny-skia, state stack `save()`/`restore()`, matrix transforms `translate`/`scale`, sub-path drawing operations `begin_path`/`move_to`/`line_to`/`close_path`/`fill`/`stroke`) real; `MediaPipeline` is a playback state machine only — no Media Foundation bindings, no audio/video decode.
...
Objective: resumable file downloads with Windows Mark-of-the-Web protection. Components: `downloads`, `networking`, `platform-windows`, `storage`. Depends on: M19. Tasks: `DownloadManager` (tokio HTTP streaming to disk, progress tracking, cancel), MOTW `Zone.Identifier` attachment on completion, RFC 6266 `Content-Disposition` (standard & extended UTF-8 `filename*`) parsing and collision resolution (`find_available_path`). Tests: download-lifecycle and MOTW tests. DoD: downloading a URL writes the file and tags it with `Zone.Identifier`. Risks: low (reuses the M4 HTTP client). NOT yet: download resumption across restarts, quota/safety heuristics.

**M21 — Compositor & Performance Benchmarks *(renumbered)* — Status: Partial**
Objective: compositor layer/damage infrastructure plus a repeatable pipeline benchmark harness. Components: `compositor`, `raster`, `benchmarks`. Depends on: M20. Tasks: damage tracking (dirty-rect accumulation), layer composition with transforms/opacity, GPU partial damage texture uploads (`upload_rgba_rect`), `GpuCompositor::composite_layers_with_damage`, and full html→raster pipeline timing benchmark. Tests: compositor unit tests, benchmark-harness test. DoD: layers composite with correct damage; the benchmark harness produces per-stage timings. Risks: low. NOT yet: wgpu GPU compositing (M10b still outstanding), profile-driven optimization against the §28 targets on real sites (the original M21 performance intent), WPT compatibility.

**M22 — Security Hardening**
**Status: Partial.** CSP directive parsing (default/script/style/connect/img-src + source matching), DPAPI encryption (`CryptProtectData`), Windows 11 `AppsUseLightTheme` registry theme query in `platform-windows`, and a `PrivateBrowsing` ephemeral profile are real; CORS + mixed-content enforced on all live subresources (`<img>`, `<link rel="stylesheet">`, `<script src="...">` fetches go through `fetch_with_security_context` against document origin). Not implemented: CSP nonce/hash sources and violation reporting, remainder of the §23 table.
...
**Status: Partial.** `soul-shell` wires the full engine pipeline and all subsystem crates; workspace builds and tests green (124+ unit & integration tests passing). Not implemented: code signing, updater, crash-report pipeline, installer, final QA per §21.
