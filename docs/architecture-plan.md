...
| M4 Networking | HTTP(S), DNS, redirects, cookies | **Partial** — real HTTP/1.1 + TLS 1.2/1.3 (hyper 1, rustls, webpki-roots) wired into shell live navigation; bounded redirect following (5 hops, POST→GET on 301/302/303); `Set-Cookie` multi-header extraction and RFC 6265bis parsing into `storage::Cookie`; no hickory DNS, no HTTP/2 |
...
| M7 CSS + style | cascade, computed style | **Complete** (MVP property set) — cascade resolver parses and merges inline `<style>` and external `<link rel="stylesheet">` sheets in document order |
...
| M11 Basic JS | boa + event loop + DOM bindings | **Complete** — boa 0.21, task/microtask queues, console & DOM bindings (`document.getElementById`, `setTextContent`), wired into inline `<script>` and external `<script src="...">` execution pipeline in document order |
...
| M13 Storage | SQLite persistence | **Complete** (MVP scope) — SQLite WAL (cookies/history/bookmarks/LocalStorage) + `localStorage` and `sessionStorage` JS bindings registered and wired to script execution in `soul-shell`; `Cookie::parse` for RFC 6265bis header values |
