...
| M5 HTML parser | html5ever → DOM | **Complete** — `html5ever` TreeSink, `parse_html_with_styles`, and `parse_html_resources` (unified extractor for DOM, inline styles, stylesheet links, inline scripts, and external script URLs) |
...
| M10a Software raster | CPU pixels on screen | **Complete** (MVP scope) — `tiny-skia` software rasterizer with nested rectangular clip stack (`PushClip`/`PopClip`), multi-layer alpha blending, scaled image blits; frames reach disk as PNG via `soul-shell --output` and presented on native GPUI window |
...
| M16 Sandboxing | renderer process, Job Objects | **Partial** — `JobObject`, `JobAccounting` (active/total process query), `RestrictedToken`, `ProcessLauncher` real; launcher job-lifetime bug fixed via `SandboxedChild` (job kept alive, `kill_job` tested); restricted token still never applied, nothing spawned outside tests |
...
| M18 Media | MF playback + Canvas 2D | **Partial** — `ImageDecoder` (PNG/JPEG/WebP/GIF/BMP/SVG with `decode_auto` format sniffing and multi-frame animation delay extraction), `Canvas2DContext` (state stack `save()`/`restore()`, transforms, path builder `move_to`/`line_to`/`close_path`/`fill`/`stroke`, rect fill/stroke/clear) real; `MediaPipeline` = state machine + solid-color `generate_frame`, no Media Foundation decode |
...
| M22 Security | CSP, DPAPI, private browsing | **Partial** — `CspPolicy` directive parse, `Dpapi`, Windows 11 `AppsUseLightTheme` registry theme query, `PrivateBrowsing` real; CORS + mixed-content enforced on all live subresources (`<img>`, `<link rel="stylesheet">`, `<script src="...">` fetches go through `fetch_with_security_context` against document origin) |
