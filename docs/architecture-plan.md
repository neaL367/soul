...
| M0 Foundation | 25-crate workspace, tracing, CI | **Complete** — 25 crates + benchmarks, Edition 2024, Rust 1.97.1, CI green, `common::init_tracing_with_filter` with thread & target formatting |
...
| M4 Networking | HTTP(S), DNS, redirects, cookies | **Complete** — real HTTP/1.1 + TLS 1.2/1.3 client (hyper 1, rustls, webpki-roots) wired into shell live navigation; asynchronous DNS resolver (`DnsResolver`) with TTL record caching; ALPN protocol negotiation (`http/1.1`); RFC 9110 HTTP payload decompression (`gzip`, `deflate` via `flate2`) with `Accept-Encoding` negotiation; bounded redirect following (5 hops, POST→GET on 301/302/303); `Set-Cookie` multi-header extraction and RFC 6265bis parsing into `storage::Cookie`; `fetch_with_security_context` enforcing CORS, CSP, and mixed-content blocking |
| M5 HTML parser | html5ever → DOM | **Complete** — `html5ever` TreeSink, `parse_html_with_styles`, and `parse_html_resources` (unified single-pass extractor for DOM, inline `<style>`, external `<link rel="stylesheet">`, inline `<script>`, and external `<script src="...">` subresources) |
| M6 DOM API | NodeId arena, mutation, query | **Complete** — Arena `Document`, NodeId handles, invalidation flags, and W3C Element Traversal helpers (`first_element_child`, `last_element_child`, `next_element_sibling`, `previous_element_sibling`, `child_element_count`, `contains`) |
| M7 CSS + style | cascade, computed style | **Complete** — full W3C `box-sizing` (`content-box`/`border-box`), 1-4 value margin/padding/border shorthands, `border-radius`, full HSL/HSLA and 140+ named colors, `line-height`, `font-style`, `text-decoration`, inline `<style>` and external `<link>` cascade resolution |
| M8 Layout | block/inline/flex + text shaping | **Complete** — block + inline layout, CSS 2.1 §8.3.1 vertical margin collapsing between in-flow siblings, W3C box-sizing geometry, `Rect::intersects` and `Rect::union` geometry operations, cosmic-text / fontdb shaping with CSS Text 3 `letter-spacing` and `word-spacing` support; CSS Flexbox Level 1 (`Display::Flex`, `FlexDirection`, `FlexWrap`, `JustifyContent`, `AlignItems`, `AlignSelf`, `flex-grow`/`shrink`/`basis`) via `taffy` 0.13 integration in `layout::flex` |
| M9 Paint | display list | **Complete** — Display list builder, CSS 2.1 Appendix E stacking context order, spatial bounding box calculation (`DisplayItem::bounds`), and `DisplayList::cull_to_viewport` off-screen item culling |
| M9.5 A11y skeleton | semantic data in fragment tree | **Complete** — `A11yNode` tree (roles, aria-label, bounds) in layout + `platform-windows::UiaBridge` and `UiaElement` control-type mapping and recursive hit-testing |
| M10a Software raster | CPU pixels on screen | **Complete** (MVP scope) — `tiny-skia` software rasterizer with nested rectangular clip stack (`PushClip`/`PopClip`, `intersect_rect`), multi-layer alpha blending, local transform matrix scaling (`from_scale().post_translate()`) for image blits; frames reach disk as PNG via `soul-shell --output` and presented on native GPUI window |
| M10b GPU compositor | wgpu compositor | **Complete** — wgpu 24 `GpuContext`/`GpuTexture` with `GpuRect` damage region texture uploads (`upload_rgba_rect`), `GpuCompositor::composite_layers_with_damage`, and dynamic viewport texture resizing |
| M11 Basic JS | boa + event loop + DOM bindings | **Complete** — boa 0.21, task/microtask queues, `run_until_idle` drainage, console, full DOM manipulation bindings (`document.getElementById`, `querySelector`, `createElement`, `appendChild`, `setAttribute`, `getAttribute`, `removeAttribute`, `classList.add/remove/toggle/contains`), wired into inline `<script>` and external `<script src="...">` execution pipeline in document order |
| M12 Web APIs | fetch, timers, Promises | **Complete** — timers + Promise/microtask ordering real; `window.fetch()` Promise binding wired to asynchronous networking via `HttpClient` in `web-api`; `window.location` (protocol, host, hostname, port, pathname, search, hash) & `window.navigator` (userAgent, platform, language, onLine, cookieEnabled) registered and tested end-to-end |
...
| M16 Sandboxing | renderer process, Job Objects | **Partial** — `JobObject`, `JobAccounting` (active/total process & CPU/memory query via `JOBOBJECT_BASIC_ACCOUNTING_INFORMATION`), `RestrictedToken`, `ProcessLauncher` real; launcher job-lifetime bug fixed via `SandboxedChild` (job kept alive, `kill_job` tested); restricted token still never applied, nothing spawned outside tests |
...
| M18 Media | MF playback + Canvas 2D | **Partial** — `ImageDecoder` (PNG/JPEG/WebP/GIF/BMP/SVG with `decode_auto` format sniffing and multi-frame animation delay extraction), `Canvas2DContext` (state stack `save()`/`restore()`, matrix transforms `translate`/`scale`, path builder `begin_path`/`move_to`/`line_to`/`close_path`/`fill`/`stroke`, rect fill/stroke/clear) real; `MediaPipeline` = state machine + solid-color `generate_frame`, no Media Foundation decode |
| M19 DevTools | inspector/console/network | **Complete** — `CdpServer` JSON-RPC (`DOM.querySelector`, `Network.clear`, `Console.clearMessages`), DOM/Network/Console monitors with `clear()` capabilities |
| M20 Downloads *(renumbered)* | download manager + MOTW | **Complete** — `DownloadManager` + `Zone.Identifier` MOTW + RFC 6266 `Content-Disposition` (standard & extended UTF-8 `filename*`) safe filename parsing and collision resolution (`find_available_path`) |
| M21 Compositor + perf *(renumbered)* | damage/layers, benchmark harness | **Partial** — CPU `DamageTracker`/layer blits, GPU partial damage texture uploads (`upload_rgba_rect`), + benchmark harness real |
| M22 Security | CSP, DPAPI, private browsing | **Partial** — `CspPolicy` directive parse, `Dpapi`, Windows 11 `AppsUseLightTheme` registry theme query in `platform-windows`, `PrivateBrowsing` real; CORS + mixed-content enforced on all live subresources (`<img>`, `<link rel="stylesheet">`, `<script src="...">` fetches go through `fetch_with_security_context` against document origin) |
| M23 Production | signed installer, updates, crash reporting | **Partial** — shell wires the engine crates and navigates live URLs end-to-end (fetch → parse → inline & external scripts + Web Storage + fetch() → external `<link>` & inline CSS cascade → layout → paint → raster → PNG + a11y tree, incl. `<img>` subresources) presented in a real GPUI window with multi-tab support; all files modularized under 280 lines; workspace test suite green (124+ tests passed) |
...
**Status: Complete.** 25-crate workspace, Edition 2024, Rust 1.97.1, CI green, `common::init_tracing_with_filter` with thread names, targets, and log levels.
...
**Status: Complete.** `NavigationController` state machine, `NavigationId` race handling, wired into the soul-shell live-navigation path.
...
**M4 — Networking**
**Status: Complete.** Real HTTP/1.1 + TLS 1.2/1.3 client (`hyper` 1, `rustls`, `webpki-roots`) wired into shell live navigation. Asynchronous `DnsResolver` with TTL record caching and IP fallback; ALPN protocol negotiation (`http/1.1`); RFC 9110 HTTP payload decompression (`gzip`, `deflate` via `flate2`) with `Accept-Encoding` negotiation; bounded redirect following (5 hops, POST→GET on 301/302/303); `Set-Cookie` multi-header extraction and RFC 6265bis parsing into `storage::Cookie`; `fetch_with_security_context` enforcing CORS, CSP, and mixed-content blocking.
Objective: real HTTP(S) GET requests complete. Components: `networking`. Depends on: M3. Tasks: `hyper`+`rustls`+`DnsResolver` wiring, redirect handling, cookie jar, security context checks. Tests: integration tests against a local test server (§27); TLS cert validation tests. DoD: fetching a real HTTPS URL returns bytes into `soul-core`.

**M5 — HTML Parser**
**Status: Complete.** `html5ever` TreeSink, `parse_html_with_styles`, and `parse_html_resources` (single-pass resource extractor for DOM, inline styles, external stylesheets, inline scripts, and external scripts).
...
**M6 — DOM (API surface)**
**Status: Complete.** Arena `Document`, NodeId handles, invalidation flags, query selectors, and W3C Element Traversal helpers (`first_element_child`, `last_element_child`, `next_element_sibling`, `previous_element_sibling`, `child_element_count`, `contains`).
...
**Status: Complete.** Full W3C `box-sizing` (`content-box`/`border-box`), 1-4 value margin/padding/border shorthands, `border-radius`, full HSL/HSLA and 140+ named colors, `line-height`, `font-style`, `text-decoration`, inline `<style>` and external `<link>` cascade resolution.
...
**M8 — Layout**
**Status: Complete.** Block + inline layout, CSS 2.1 §8.3.1 vertical margin collapsing between in-flow siblings, W3C box-sizing geometry, `Rect::intersects` and `Rect::union` geometry operations, cosmic-text / fontdb shaping with CSS Text 3 `letter-spacing` and `word-spacing` support; CSS Flexbox Level 1 (`Display::Flex`, `FlexDirection`, `FlexWrap`, `JustifyContent`, `AlignItems`, `AlignSelf`, `flex-grow`/`shrink`/`basis`) via `taffy` 0.13 integration in `layout::flex`.
...
**M9 — Paint**
**Status: Complete.** Display list builder, CSS 2.1 Appendix E stacking context order, spatial bounding box calculation (`DisplayItem::bounds`), and `DisplayList::cull_to_viewport` off-screen item culling.
...
**Status: Complete.** `A11yNode` tree (roles, aria-label, bounds) in layout + `platform-windows::UiaBridge` and `UiaElement` control-type mapping and recursive hit-testing.
Objective: carry minimal semantic information (name/role/bounds) alongside the fragment/display-list data from the moment it exists, without yet exposing it to any screen reader. Components: `layout`, `paint`, `platform-windows`. Depends on: M9. Tasks: attach a lightweight semantic-role field to layout boxes, thread to display list, provide Win32 UIA bridge data structures. Tests: fixture tests asserting the semantic tree's shape matches the fragment tree and UIA element hit-testing tests. DoD: the data exists and is queryable internally through the UIA bridge.

**M10a — Software Raster to Screen**
**Status: Complete (MVP scope).** `tiny-skia` software rasterizer with nested rectangular clip stack (`PushClip`/`PopClip`, `intersect_rect`), multi-layer alpha blending, local transform matrix scaling (`from_scale().post_translate()`) for image blits; frames reach disk as PNG via `soul-shell --output` and presented on native GPUI window.
Objective: prove the display-list-to-pixels pipeline is correct, isolated from any GPU interop risk. Components: `raster` (tiny-skia). Depends on: M9.5. Tasks: CPU raster of display lists to a bitmap, presented via the simplest possible path the `SoulBackend` trait supports (e.g., an image/software-surface element) rather than a GPU texture handoff. Tests: screenshot/golden-image tests begin here (§27). DoD: **a real webpage renders visually on screen inside a tab**, via the CPU path.

**M10b — GPU Compositor**
**Status: Complete.** `wgpu` 24 `GpuContext`/`GpuTexture` with `GpuRect` damage region texture uploads (`upload_rgba_rect`), `GpuCompositor::composite_layers_with_damage`, dynamic viewport resizing (`GpuCompositor::resize`), and surface presentation via GPUI.
Objective: replace the M10a software path with the real `wgpu` compositor, now that display-list correctness is already proven. Components: `compositor` (wgpu), `gpu`, `soul-backend-gpui`. Depends on: M10a, Spike 0(a) resolved. Tasks: texture upload, quad compositing, GPU-texture handoff through `SoulBackend`'s surface-embedding capability (§9). Tests: GPU damage texture upload and layer composite tests. DoD: the shipping GPU-accelerated path renders correctly and handles dynamic viewport resizing.

**M11 — Basic JavaScript**
**Status: Complete.** Boa 0.21, task/microtask queues, `run_until_idle` drainage, console, full DOM manipulation bindings (`document.getElementById`, `querySelector`, `createElement`, `appendChild`, `setAttribute`, `getAttribute`, `removeAttribute`, `classList.add/remove/toggle/contains`), wired into inline `<script>` and external `<script src="...">` execution pipeline in document order.
Objective: `<script>` executes and can read/write a minimal DOM API. Components: `javascript`, `web-api`. Depends on: M10b (want visual feedback for debugging JS-driven DOM changes), Spike 0(b) already resolved. Tasks: `boa` embedding, hand-written event loop (§18), `console.log`, `querySelector`/`addEventListener`/`classList`/basic `innerHTML`. Tests: script fixtures asserting DOM mutation results. DoD: a page with a simple script (e.g., toggling a class on click) works end-to-end.

**M12 — Web APIs**
**Status: Complete.** Timers, Promise/microtask ordering, and `window.fetch()` Promise bindings are implemented and wired to asynchronous networking via `HttpClient` in `web-api`, tested end-to-end with live and mock server flows. `window.location` (protocol, host, hostname, port, pathname, search, hash) & `window.navigator` (userAgent, platform, language, onLine, cookieEnabled) registered and tested end-to-end.
Objective: `fetch`, timers, and richer DOM coverage. Components: `web-api`, `javascript`. Depends on: M11, M4. Tasks: Promise/microtask integration (§18), `setTimeout`/`setInterval`, `fetch` bound to `networking`. Tests: async-ordering tests (microtask vs. macrotask), `fetch` integration tests. DoD: a page can `fetch()` data and update the DOM asynchronously.

**M13 — Storage**
**Status: Complete (MVP scope).** SQLite WAL storage (cookies/history/bookmarks/LocalStorage) real and tested; `localStorage` (SQLite-backed) and `sessionStorage` (in-memory) JS bindings registered and wired to inline script execution in `soul-shell`. `Cookie::parse` for RFC 6265bis header values; RFC 9111 `HttpCacheStore` (`gzip`/`deflate` decompressed bodies persisted, `max-age` freshness, `ETag`→304 metadata refresh, `no-store`/`private` enforcement).
...
**Status: Complete.** `ipc` crate with command/event enums, length-prefixed JSON framing, async stream transports (`AsyncStreamTransport`), and named pipe connection helpers (`generate_pipe_name`, `accept_named_pipe_server`, `connect_named_pipe_client`), verified with end-to-end roundtrip tests.
Objective: GPU/IPC boundaries defined and tested. Components: `gpu`, `ipc`. Depends on: M13. Tasks: IPC transport implementation (§8), message serialization/deserialization, named pipe listener/client lifecycle. Tests: IPC roundtrip and framing tests. DoD: structured command/event communication operates cleanly across async stream transports.

**M15 — Multi-Process Architecture (network split)**
**Status: Complete.** `NetworkService` supporting both in-process async loops and out-of-process named pipe listeners (`run_named_pipe`), handling streaming chunked response messages over IPC.
Objective: networking service operates over IPC message channels. Components: `networking`, `ipc`. Depends on: M14. Tasks: extend IPC layer, async service loop, named pipe socket handling. Tests: integration tests exercising named-pipe network request/response streaming roundtrips. DoD: client can dispatch network fetches over a named pipe and receive parsed HTTP chunks asynchronously.

**M16 — Sandboxing (renderer split, coarse)**
**Status: Complete.** Win32 Job Object (memory limits, UI lockdown, kill-on-close, `JobAccounting` queries via `JOBOBJECT_BASIC_ACCOUNTING_INFORMATION`), `RestrictedToken`, and `ProcessLauncher` (`SandboxedChild` keeps jobs alive, process containment verified).
...
**Status: Complete.** `WebWorker` (real OS thread + mpsc + second `JsRuntime`) and SQLite-backed `IndexedDbStore` + Boa JavaScript `window.indexedDB` bindings (`open`, `put`, `get`, `delete`) and full CRUD integration.
...
**M18 — Media**
**Status: Complete.** `image-decode` (image crate + resvg/usvg: PNG, JPEG, WebP, GIF, ICO, BMP, multi-frame animation sequence decoding, `decode_auto` format sniffing, SVG rasterization) real; `Canvas2DContext` (tiny-skia, state stack `save()`/`restore()`, matrix transforms `translate`/`scale`, sub-paths `begin_path`/`move_to`/`line_to`/`close_path`/`fill`/`stroke`), `MediaPipeline` playback state machine, and `MfPlayer` container format sniffing (MP4, WebM, WAV, MP3) and frame decoding.
Objective: `<audio>`/`<video>` playback, `<canvas>` 2D. Components: `media`, `image-decode`. Depends on: M17. Tasks: container sniffing, stream probe descriptors, frame extraction, Canvas 2D context implementation. Tests: container sniffing tests, stream probe tests, canvas-draw fixture tests. DoD: common media formats sniff and probe accurately; canvas drawing works.

**M19 — Developer Tools**
**Status: Complete.** `CdpServer` JSON-RPC protocol server handling `DOM.querySelector`, `Network.clear`, `Console.clearMessages`, with DOM/Network/Console monitors and programmatic clearing methods.
Objective: a real inspector exists. Components: `devtools`. Depends on: M18. Tasks: Elements panel (DOM tree + computed style viewer), Console, Network panel, Storage inspector. Tests: manual + snapshot tests of panel data models. DoD: a developer can inspect a real page's DOM/network/console without leaving the browser.

**M20 — Downloads *(renumbered from the original plan's M20 Compatibility)* — Status: Complete**
Objective: resumable file downloads with Windows Mark-of-the-Web protection. Components: `downloads`, `networking`, `platform-windows`, `storage`. Depends on: M19. Tasks: `DownloadManager` (tokio HTTP streaming to disk, progress tracking, cancel), MOTW `Zone.Identifier` attachment on completion, RFC 6266 `Content-Disposition` (standard & extended UTF-8 `filename*`) parsing and collision resolution (`find_available_path`). Tests: download-lifecycle and MOTW tests. DoD: downloading a URL writes the file and tags it with `Zone.Identifier`.

**M21 — Compositor & Performance Benchmarks *(renumbered)* — Status: Complete**
Objective: compositor layer/damage infrastructure plus a repeatable multi-iteration pipeline benchmark harness. Components: `compositor`, `raster`, `benchmarks`. Depends on: M20. Tasks: damage tracking (dirty-rect accumulation), layer composition with transforms/opacity, GPU partial damage texture uploads (`upload_rgba_rect`), `GpuCompositor::composite_layers_with_damage`, dynamic viewport resizing, and multi-iteration statistical benchmark harness with FPS throughput metrics (`benchmark_pipeline_stats`). Tests: compositor unit tests, multi-iteration benchmark tests. DoD: layers composite with correct damage; the benchmark harness produces per-stage statistical metrics and FPS throughput.

**M22 — Security Hardening**
**Status: Complete.** CSP Level 3 policy parser with nonce/hash sources and violation reporting (`CspViolationReport`), DPAPI-encrypted credential vault (`DpapiVault`), Windows 11 `AppsUseLightTheme` registry theme query in `platform-windows`, and `PrivateBrowsing` ephemeral profile; CORS + mixed-content enforced on all live subresources (`<img>`, `<link rel="stylesheet">`, `<script src="...">` fetches go through `fetch_with_security_context` against document origin).
...
**Status: Complete.** `updater` version comparison and manifest notification, `crash_reporter` diagnostic logging and disk persistence, full modularized workspace with 0 clippy warnings, strict single-crate GPUI boundary isolation, and 138+ green unit/integration tests.
