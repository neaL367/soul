...
**Status: Partial.** Raw GPUI elements now provide Soul toolbar buttons and an from-scratch omnibox; keyboard events edit omnibox text and Enter emits `SoulEvent::OmniboxSubmitted`; live frame polling refreshes post-start navigation frames. `SoulEvent` reaches `soul-shell`, which performs background navigation. Outstanding: full `InputRouter` integration, wheel/resize/DPI event forwarding, and page hit-testing.
