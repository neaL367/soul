...
| M2 Window + input | OS input events, DPI | **Partial** — raw GPUI mouse/keyboard/wheel events route through `InputRouter`; GPUI bounds and scale factor emit `SoulEvent::WindowResized`; omnibox editing/navigation works; Win32 input smoke test passes; page hit-testing remains |
